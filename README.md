...
缓存数据
 ...
 - 缓存数据的类别
   ...
   - object 模型对象
     ...
     1）获取查询结果 result = User.query.join().......

     2）获取查询的sql语句并且使用md5计算得到值: xxxxsdsadakk 这个值作为存储到redis数据的key

     3）设置缓存：setex(md5结果作为key, 过期时长, result) 

     4）下次如果是同一个查询，md5值是一致的，那么就可以根据key提取缓存值value

   - 一个视图的响应结果

     1）不管视图函数执行多少次查询，最终以路由路径作为key，结果作为value 存储起来

     2）下次访问的同一个接口统一个资源，就可以根据路由作为key，提取缓存数据。
